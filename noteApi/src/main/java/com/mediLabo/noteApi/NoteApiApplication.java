...
public class NoteApiApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		if (noteRepository.findAll().isEmpty()) {
			noteRepository.save(Note.builder().fkPatientNom("TestNone")
					.note("Le patient déclare qu'il 'se sent très bien' Poids égal ou inférieur au poids recommandé")
					.build());

			noteRepository.save(Note.builder().fkPatientNom("TestBorderline").note(
			...
			noteRepository.save(Note.builder().fkPatientNom("TestInDanger")
					.note("Le patient déclare qu'il fume depuis peu").build());

			noteRepository.save(Note.builder().fkPatientNom("TestInDanger").note(
			...
			noteRepository.save(Note.builder().fkPatientNom("TestEarlyOnset").note(
			...
			noteRepository.save(Note.builder().fkPatientNom("TestEarlyOnset")
					.note("Le patient déclare qu'il a mal au dos lorsqu'il reste assis pendant longtemps").build());
			...
			noteRepository.save(Note.builder().fkPatientNom("TestEarlyOnset")
					.note("Taille, Poids, Cholestérol, Vertige et Réaction").build());
		}
	}
}
